...
type SQSService interface {
	SendMessage(queueName *string, message *[]byte, additionalAttributes map[string]string) (*string, error)
	SendMessageWithDelay(queueName *string, message *[]byte, delaySeconds int64, additionalAttributes map[string]string) (*string, error)
	ReceiveMessages(queueName *string, maxMessages int64, waitTimeSeconds int64, additionalAttributes ...string) ([]SQSMessage, error)
...
func (s *sqsServiceImpl) SendMessage(queueName *string, message *[]byte, additionalAttributes map[string]string) (*string, error) {
	return s.SendMessageWithDelay(queueName, message, 0, additionalAttributes)
}

func (s *sqsServiceImpl) SendMessageWithDelay(queueName *string, message *[]byte, delaySeconds int64, additionalAttributes map[string]string) (*string, error) {
	...
	for key, val := range additionalAttributes {
...
func (s *sqsServiceImpl) ReceiveMessages(queueName *string, maxMessages int64, waitTimeSeconds int64, additionalAttributes ...string) ([]SQSMessage, error) {
	...
	for index, message := range resp.Messages {
		...
		for _, attribute := range additionalAttributes {
			val, ok := message.MessageAttributes[attribute]
			if !ok {
				continue
			}
			messages[index].AdditionalAttributes[attribute] = val.StringValue
		}
...
func (s *sqsServiceImpl) getQueueURL(queueName string) (*string, error) {
	s.lock.RLock()
	queueUrl, ok := s.urlCache[queueName]
	s.lock.RUnlock()
	if !ok {
